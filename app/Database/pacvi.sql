...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 16, 2022 at 01:21 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `P_INSERTAR_EVENTO` (IN `P_ID_EVENT` INT, IN `P_ENCARGADO` VARCHAR(200), IN `P_NOMBRE_EVENTO` VARCHAR(100), IN `P_IMG_PATH` VARCHAR(500), IN `P_DSC_EVENTO` VARCHAR(100), IN `P_FECHA_EVENTO` DATE, IN `P_FECHA_ACTUAL` DATE)   BEGIN
            INSERT INTO EVENTOS     
            (
                ID_EVENT,
                ENCARGADO, 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `P_INSERTAR_GALERIA` (IN `P_IMG_PATH` VARCHAR(300), IN `P_FECHA` DATE)   BEGIN
            INSERT INTO GALERIA     
            (
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `P_INSERTAR_PERSONA` (IN `P_ID_REGISTRO_PERSONA` VARCHAR(50), IN `P_ID_PERSONA` VARCHAR(30), IN `P_NOMBRE` VARCHAR(30), IN `P_APPELLIDOS` VARCHAR(60), IN `P_FECH_NACIMIENTO` DATE, IN `P_GENERO` VARCHAR(2), IN `P_TIPO_PERSONA` INT(1))   BEGIN
            INSERT INTO PERSONA     
            (
                ID_REGISTRO_PERSONA, 
                ID_PERSONA, 
...
(1, 'JUAN PEREZ', 'APOYO A LOS NIÑOS', '../../', 'REGALOS PARA LOS NIÑOS', '2022-07-15', '2022-07-15');
...
(2, 'ESTA AQUI', '2022-05-12'),
(3, 'ESTA AQUI', '2022-05-12');
...
('PER_1', 'RANDALL DE JESUS', 'GUZMAN ', '1999-09-27', 'M', 1, 'HOLA');
...
(2, 'ran@676gmail.com', '83195333', 'PER_1');
